...
void RecordingsManagerWidget::initPlaybackTools ()
{
    ...
    playbackBar = new PlaybackBar(recording);
...
{
    // Removes recordings that do not exist anymore

    recordingsList->blockSignals(true);
    ushort i(0);
    while (i < recordingsList->count())
    {
        if (QFile::exists(recordingsList->item(i)->text()))
            i++;
        else
            delete recordingsList->takeItem(i);
    }
    recordingsList->blockSignals(false);
...
void RecordingsManagerWidget::rename ()
{
    ...
    bool ok;
    QString newFileName(QInputDialog::getText(this, tr("Rename file"), tr("Please input the new file name :"), QLineEdit::Normal, QFileInfo(fileName).completeBaseName(), &ok).remove(QRegularExpression("^ +")).remove(QRegularExpression(" +$")));
    if (!ok)
        return;
